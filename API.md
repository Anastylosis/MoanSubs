...
`authorship`/`credited_to` (migration 0026, feature `authorship`) are
additive too. `authorship` is one of a closed vocabulary — `shared`
(default: the uploader is passing along a file they found, no claim
either way), `credited` (the uploader authored it and wants public
credit), `uncredited` (the uploader authored it and declines public
credit) — declared, not enforced, the same posture as `kind`. `credited_to`
is the uploader's account name, present **only** when `authorship` is
`credited`; omitted (not sent as `""`) for `shared` and, CRITICALLY, for
`uncredited` — an uncredited track's uploader is never named on any public
response or page (see the catalogue's `/u/{name}` page documentation in
MANUAL.md), because that would be exactly the credit they declined to
take. A second upload of the identical body under a different `authorship`
corrects the existing track's `authorship` in place, the same re-upload
rule `kind` follows.
...
`authorship`, `credited_to` (migration 0026; `credited_to` omitted unless
`authorship` is `"credited"` — same rule as the release shape's own
`tracks[].credited_to`).
...
`/u/{name}` page — see the release shape's own `authorship`/`credited_to`
documentation above. Re-uploading a byte-identical subtitle under a
different `authorship` corrects the existing track's authorship in place,
exactly like `kind` above; omitting the field on a re-upload leaves the
stored value alone rather than resetting it to `shared`.
