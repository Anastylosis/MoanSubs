...
`{"version": "<semver or dev>", "features": ["lookup", "match", "withdraw", "stats", "srt", "votes", "stash_ids", "metadata", "kinds", "revisions", "titles", "trending", "fit", "credits", "authorship"],
...
`generated_source` (migration 0026, feature `authorship`) is additive too,
present only when `generated` is `true`: `"provenance"` when the tool's own
marker was detected in the raw upload (with structured tool/model metadata
behind it — see `provenance` on `GET /api/v1/subtitles/{id}` below),
`"declared"` when the only reason `generated` is `true` is the uploader's
own checkbox at upload time. Detection wins the label whenever both are
true — the marker-backed claim is strictly stronger evidence than a bare
declaration, and a client should render the two differently (the plugin's
own badge and this node's release page both do): `"declared"` is worth
noting but confirms nothing, the way `"provenance"` does. `generated`
itself has been detection-or-declaration since this feature shipped, never
just detection alone — see `POST /api/v1/subtitles` below for why a
declaration can only ever add the label, never remove it.

`credited_to` (migration 0026, feature `authorship`) is additive too — the
uploader's account name, present **only** when the track's authorship is
`credited`; omitted (not sent as `""`) otherwise. This is the ONLY public
trace of authorship: the `authorship` value itself (`shared`/`credited`/
`uncredited`, a closed vocabulary — declared, not enforced, the same
posture as `kind`) is deliberately **not** a field on any public response
anywhere, including this one and `GET /api/v1/subtitles/{id}` below — only
set at upload time (`POST /api/v1/subtitles`, below) and shown on the
node's mod pages (MANUAL.md), which already show the uploader's identity.
An anonymous caller who can enumerate ids must not be able to learn a
track's authorship value at all, which unconditionally exposing it would
let them do for exactly the `uncredited` tracks that value exists to hide
— see the catalogue's `/u/{name}` page documentation in MANUAL.md for the
...
client's job at write time), `generated`, `generated_source` (migration
0026; `"provenance"`/`"declared"`, omitted unless `generated` is `true` —
see the release shape's own documentation above), `provenance` (structured
JSON when the track was machine-generated by a tool that records it —
every string field in it is capped at 200 runes and stripped of control
characters, since the whole object comes from an uploader-controlled NOTE
block),
`license`, `source`, `downloads`, `up`, `down`, `kind`, `kind_label`
(migration 0021; `kind_label` omitted unless `kind` is `other`),
`credited_to` (migration 0026; same rule as the release shape's own
`tracks[].credited_to` — present only for a `credited` track). This is a
public, anonymously-readable endpoint, so — same as the release shape —
the track's raw `authorship` value itself is never a field here; only
...
{"oshash": "7a604bd1a3800e67", "phash": "ff00454c6e3f1333",
 ...
 "stash_ids": [{"endpoint": "https://stashdb.org/graphql",
                "stash_id": "c72cba4a-1e2b-4f0e-8f3a-1234567890ab"}],
 "kind": "sdh", "kind_label": "",
 "authorship": "credited", "generated": false}
```
...
`authorship` (migration 0026, feature `authorship`) is optional, defaulting
to `shared` when omitted, and must be one of `shared`/`credited`/
`uncredited` — an unknown value refuses the whole upload with
`400 {"error": "authorship: must be one of shared, credited, uncredited"}`.
`shared` makes no claim (the default: you're passing along a file you
found); `credited` shows your account name as `credited_to` wherever this
track appears publicly; `uncredited` records that you authored it (visible
to moderators) but never surfaces publicly anywhere, including your own
`/u/{name}` page — see the release shape's own `credited_to`
...
meaningful.** It is OR'd into the stored `generated` flag alongside marker
...
costs nothing, so there is no reason not to be honest about it. See
`generated_source` (the release shape, above) for how a response tells a
marker-backed detection apart from a bare declaration.
