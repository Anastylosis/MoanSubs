...
func (s *Server) fitPairing(ctx context.Context, trackID, releaseID int64) (*store.SubtitleTrack, *apiError) {
	...
	if releaseID != track.ReleaseID {
		...
		if errors.Is(err, store.ErrNotFound) {
			return nil, &apiError{http.StatusBadRequest, "no release with that id"}
		}
		if err != nil {
	...
	if !valid {
		return nil, &apiError{http.StatusBadRequest, "release_id is not a pairing this track offers: must be the track's own release or a sibling release in the same work"}
	}
	return track, nil
}
...
func (s *Server) castFit(ctx context.Context, account *store.Account, trackID int64, req fitRequest) (*fitResponse, *apiError) {
	if !s.FitLimiter.Allow(strconv.FormatInt(account.ID, 10)) {
		return nil, &apiError{http.StatusTooManyRequests, "fit rate limit exceeded"}
	}
	if req.ReleaseID <= 0 {
		return nil, &apiError{http.StatusBadRequest, "release_id is required"}
	}
...
func (s *Server) retractFit(ctx context.Context, account *store.Account, trackID, releaseID int64) *apiError {
	if !s.FitLimiter.Allow(strconv.FormatInt(account.ID, 10)) {
		return &apiError{http.StatusTooManyRequests, "fit rate limit exceeded"}
	}
