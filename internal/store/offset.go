...
type SiblingTrack struct {
	...
	Fits    int
	Misfits int
	// DeclaredGenerated: migration 0026 (WP-authorship), OR'd into
	// Generated by the caller for display — see SubtitleTrackSummary's own
	// comment (subtitle_track.go).
	DeclaredGenerated bool
	// Provenance: same raw jsonb, same reason, as SubtitleTrackSummary's
	// own Provenance field — a sibling's badge needs it too.
	Provenance []byte
}
...
func (s *Store) SiblingTracks(ctx context.Context, releaseID int64) ([]SiblingTrack, error) {
	rows, err := s.pool.Query(ctx, `
		SELECT t.id, t.release_id, t.lang, t.generated, t.downloads,
		       ...
		       t.declared_generated, t.provenance
		FROM releases self
		JOIN releases sib
	...
	for rows.Next() {
		...
		if err := rows.Scan(&st.TrackID, &st.ReleaseID, &st.Lang, &st.Generated,
			...
			&st.Fits, &st.Misfits, &st.DeclaredGenerated, &st.Provenance); err != nil {
